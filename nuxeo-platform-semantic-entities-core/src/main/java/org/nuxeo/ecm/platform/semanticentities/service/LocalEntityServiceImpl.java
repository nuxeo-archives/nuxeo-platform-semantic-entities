...
public class LocalEntityServiceImpl extends DefaultComponent implements LocalEntityService {
    ...
    protected Cache<String, DocumentRef> recentlyDereferenced = CacheBuilder.newBuilder().concurrencyLevel(4).expireAfterWrite(
            5, TimeUnit.SECONDS).build();

    protected Cache<DocumentRef, String> progressMessages = CacheBuilder.newBuilder().concurrencyLevel(4).expireAfterWrite(
                ...
                public void run() {
                    if (!session.exists(ref)) {
                        ...
                        if (!ENTITY_CONTAINER_PATH.equals(createdContainer.getPathAsString())) {
                            ...
                            DocumentModel occurrenceContainer = session.createDocumentModel(parentPath, "occurrences",
                                    "OccurrenceContainer");
    ...
    public void removeOccurrences(CoreSession session, DocumentRef docRef, final DocumentRef entityRef,
        ...
        if (relatedDocuments.getCurrentPage().size() == 1 && relatedDocuments.getCurrentEntry().getRef().equals(docRef)) {
    ...
    public OccurrenceRelation addOccurrences(CoreSession session, DocumentRef docRef, DocumentRef entityRef,
        ...
        if (!session.hasPermission(docRef, Constants.ADD_OCCURRENCE_PERMISSION)) {
            ...
            throw new SecurityException(String.format("%s has not the permission to add an entity"
                    + " occurrence on document with id '%s'", session.getPrincipal().getName(), docRef));
        }
    ...
    public PageProvider<DocumentModel> getRelatedDocuments(CoreSession session, DocumentRef entityRef,
        ...
        if (!(entityRef instanceof IdRef)) {
            throw new NotImplementedException("Only IdRef instance are currently supported, got "
                    + entityRef.getClass().getName());
        }
        String query = String.format("SELECT Doc.cmis:objectId FROM %s Doc "
                + "JOIN Occurrence Occ ON Occ.relation:source = Doc.cmis:objectId "
                + "WHERE Occ.relation:target = '%s' " + "ORDER BY Doc.dc:modified DESC", documentType, entityRef);
    ...
    public List<EntitySuggestion> suggestLocalEntity(CoreSession session, String keywords, String type,
        ...
        String q = String.format("SELECT * FROM %s WHERE (ecm:fulltext_title = '%s' OR entity:normalizednames = '%s')"
                ...
                Constants.ENTITY_TYPE, sanitized, normalized, StringUtils.join(entityTypeNames, "', '"), maxSuggestions);
    ...
    public List<DocumentModel> suggestDocument(CoreSession session, String keywords, String type, int maxSuggestions) {
        ...
        String query = String.format("SELECT cmis:objectId, SCORE() relevance FROM %s " + "WHERE CONTAINS('%s')"
                + " AND cmis:objectTypeId NOT IN ('%s')" + " AND nuxeo:isVersion = false " + "ORDER BY relevance DESC",
    ...
    public DocumentModel asLocalEntity(CoreSession session, EntitySuggestion suggestion) throws
        ...
        if (suggestion.isLocal()) {
            ...
            throw new IllegalArgumentException("The provided suggestion has neither local"
